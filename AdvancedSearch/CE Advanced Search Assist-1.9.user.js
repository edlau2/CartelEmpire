...
(function() {
    ...
    var onlyReviveable = GM_getValue("onlyReviveable", false);
    ...
    function getUserStats(userId, row) {
        ...
        $.get(url, function(data, result) {
            ...
            if (result == 'success') {
                ...
                let diff = la ? now - +la : null;
                let days = diff ? parseInt(+diff / secsInDay) : null;
                let revivable = jsonObj.allowRevive;
                if (!revivable) $(row).addClass("norev");

                let node = $(row).find("td.last-act");
                $(node).text((days ? (days + " days") : '---'));

                let rep = jsonObj.reputation;
                node = $(row).find("td.rep");
                $(node).text(rep);

                if (jsonObj.status == "In Hospital") {
                    let el = $(row).find("td:last-child");
                    let color = "red";
                    if (revivable == true) {
                        displayHtmlToolTip($(el), "Accepts Revives");
                        //$(row).removeClass("norev");
                        color = "green";
                    } else {
                        if (onlyReviveable == true) $(row).addClass("hnr");
                        displayHtmlToolTip($(el), "Revives Disabled");
                        //$(row).addClass("norev");
                    }
                    $(el).css("color", color);
                }

                debug("[getUserStats] result for ", longName, `${days} old, la ${days} days, ${rep} reputation, JSON: `, jsonObj);
    ...
    function handleRevOnlyClick(e) {
        debug("handleRevOnlyClick");
        e.stopPropagation();
        e.preventDefault();

        $("tr.norev").toggleClass("hnr");
        $(this).toggleClass("cg");
        GM_setValue("onlyReviveable", ($(this).hasClass("cg") ? true : false));
    }
    ...
    function getFightDataCb(data, key) {
        ...
        if (!data)
            ...
            if ($(nameNode).length) {
                if (newest.outcome == 'Win') {
                    if (newest.type == 'Mug')
                        $(nameNode).css("color", "yellow");
                    else
                        $(nameNode).css("color", "limegreen");
                }
    ...
    function handlePageLoad(retries=0) {
        ...
        // radio btns, filter out revivable?
        let tempBtn = `<div class="btn-dr" style="float: right;">
                          <button id="reviveableOnly" class="btn btn-sm btn-dark">Reviveable</button>
                      </div>`;
        $(".header-section").append(tempBtn);
        $(".header-section").addClass("frow");

        if (onlyReviveable) $("#reviveableOnly").addClass("cg");
        $("#reviveableOnly").on('click', handleRevOnlyClick);
