...
(function() {
    ...
    var hasCeQL = ($("#CeLogo").attr("data-ceql") == "true");
    debug("hasCeQL: ", hasCeQL);
    ...
    const defAppOpts = {
        ...
        "showTopRowIcons": { on: true, desc: "Display the top-row icons, such as Town, Missions, etc, on the sidebar" },
    ...
    const townLinks = JSON.parse(GM_getValue("townLinks", JSON.stringify(defTownLinks)));
    ...
    function saveLinksTable(tblName) {
        switch (tblName) {
            case "townLinks": return GM_setValue("townLinks", JSON.stringify(townLinks));
    ...
    let townClass = GM_getValue("nav-town-pos", "fa-caret-down");
        ...
        <div id="sidebarroot" class="">
            ...
            <div id="sidebar-content">
                <div id="sb-link-list"  class="sb-area ${useLiClass}">
                    ...
                    <!------------------------ Links in the town ----------------------->
                    <li id="town-links" class="sb-row-root sb-header">Town
                        <span class="sb-caret"><i class="fa ${townClass}" data-target="nav-town"></i></span>
                    </li>

                    <ul id="nav-town" class="sb-area ${useLiClass}">
                    </ul>
    ...
    function showHideTopNav() {
        hasCeQL = ($("#CeLogo").attr("data-ceql") == "true");
        debug("hasCeQL: ", hasCeQL);
        ...
        if ($("#nav-wrap").length) // CeQL is running...
    ...
    function installSidebarContents(retries=0) {
        ...
        // Add enabled town links
        townLinks.forEach(entry => {
            if (entry.visible == true) {
                const row = `${liRow}<a href="${entry.href}">${entry.name}</a></li>`;
                $("#nav-town").append(row);
            }
    ...
    function installSettings(retries=0) {
        ...
        istallGroupOptions(townLinks, 'townGroup');
        ...
        function handleSettingsChange(e) {
            ...
            switch (tableId) {
                case "townGroup": {
                    townLinks[idx].visible = checked;
                    saveLinksTable("townLinks");
                    break;
                }
                case "marketGroup": {
    ...
    function addSettingsStyles() { GM_addStyle(`th.first { width: 800px; }`); }

    function setSidebarTopMargin(retries=0) {
        rootMargin = $("#nav-wrap").parent().height();
        if (!rootMargin) {
            rootMargin = $("div.topNav.sticky-top").height() +
                $("#contentContainer > div.row > div.col-12").height();
        }
