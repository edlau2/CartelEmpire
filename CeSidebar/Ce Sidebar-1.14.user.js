...
(function() {
    ...
    function scanTopNavIcons(init=false) {
        ...
        for (let i=0; i<$(list).length;i++) {
            let item = $(list)[i];
            if ($(item).css("color") != "rgb(255, 255, 255)" || copyAllLowerIcons == true) {
                copyToSb($(item), "mid");

                // This cycled colors, just so I knew I was grabbing the icons...
                if (colorIcons == true) {
        ...
        for (let i=0; i<$(list).length;i++) {
            let item = $(list)[i];
            if ($(item).css("fill") && $(item).css("fill").indexOf("255, 255, 255") == -1 || copyAllUpperIcons == true) {
