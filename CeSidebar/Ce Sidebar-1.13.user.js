...
(function() {
    ...
    const defAppOpts = {
        "showBotRowIcons": { on: true, desc: "Display the lower-row icons, such as Events, on the sidebar" },
        "onlyActiveBotIcons": { on: true, desc: "Only display active icons if shown on the sidebar" },
    };
    ...
    // Add icons if in alert state?
    /*
    $("#nav-wrap > div > div.row.mt-2 > div > ul > li.educationIcon.statusIcon > a > svg").css("color")
    'rgb(255, 255, 255)'
    $("#nav-wrap > div > div.row.mt-2 > div > ul > li.eventIcon.statusIcon > a > svg").css("color")
    'rgb(255, 193, 7)'
    
    let list = $("#nav-wrap > div > div.row.mt-2 > div > ul > li.statusIcon").not('.d-none').find("a > svg");
    for (let i=0; i<$(list).length;i++) { if ($($(list)[i]).css("color") != "rgb(255, 255, 255)") $($(list)[i]).css("color", "green")}
    $(list).each(function() {
    ...
    */
    ...
    function copyToSb(item, which) {
        if (!$(item).hasClass('sbCp')) {
            let target = (which == "top") ? $("#topIconWrapSb") : $("#midIconWrapSb");
    ...
    let colors = ["limegreen", "dodgerblue", "pink", "red", "purple", "yellow", "royalblue"];
    let clrIdx=0;
    function scanTopNavIcons(init=false) {
        ...
        for (let i=0; i<$(list).length;i++) {
            ...
            if ($(item).css("color") != "rgb(255, 255, 255)") {
                copyToSb($(item), "mid");
                $($(list)[i]).css("color", colors[clrIdx++]);
                if (clrIdx > colors.length - 1) clrIdx = 0;
            }
        }
        ...
        for (let i=0; i<$(list).length;i++) {
            let item = $(list)[i];
            if ($(item).css("fill") && $(item).css("fill").indexOf("255, 255, 255") == -1) {
                copyToSb($(item).parent(), "top");
                $($(list)[i]).css("fill", colors[clrIdx++]);
                if (clrIdx > colors.length - 1) clrIdx = 0;
            }
        }
    ...
    function installSettings(retries=0) {
        ...
        function installAppearanceOpts() {
            ...
            for (let idx=0; idx<$(keys).length; idx++) {
                let entry = appOpts[keys[idx]];
                let row = getSettingsRow({name: entry.desc, visible: entry.on});
