...
//                                           " valid? ", (options[key] ? isValid(options[key][pkey]) : false));
    ...
    const defOptions = {
        ...
        "lastRespectOnEnemiesPage": { "on": false, "desc": "On enemies page, display last respect from successfull attack", "visible": false, "grp": "cbt" },
...
//         });
    ...
    function getRepCallback(fightEntry, params) {
        ...
        let allEnemyNotes = $("#enemiesList > div.enemies-rows > div.row > div:nth-child(5)");
    ...
    function installRemoveEnemyBtn() {
        ...
        `);
        let allEnemyNames = $("#enemiesList > div.enemies-rows > div.row"); //$("#enemiesList > div.row:not(.row-header) > div:nth-child(1)");
        let nameHdr = $("#enemiesList > div.row-header > div:first-child");
    ...
    function addLvlOnEnemiesPage() {
        ...
        let allEnemyNames = $("#enemiesList > div.enemies-rows > div.row");
        let nameHdr = $("#enemiesList > div.row-header > div:first-child");
    ...
    function addRepOnEnemiesPage() {
        ...
        let allEnemyNames = $("#enemiesList > div.enemies-rows > div.row");
        let allEnemyNotes = $("#enemiesList > div.enemies-rows > div.row > div:nth-child(5)");
        let notesHdr = $("#enemiesList > div.row-header > div:last-child");
...
//             }
    ...
    function processEnemiesPage() {
        ...
        if (isEnemies == true) {
            ...
            let allEnemyNames = $("#enemiesList > div.enemies-rows > div.row"); //$("#enemiesList > div.row:not(.row-header) > div:nth-child(1)");
            let allEnemyNotes = $("#enemiesList > div.enemies-rows > div.row > div:nth-child(5)");
            let allEnemyStatus = $("#enemiesList > div.enemies-rows > div.row > div:nth-child(2)");

            if (Object.keys(activeStakeouts).length > 0) {
                let dirty = false;
                $(allEnemyNames).each(function(idx, item) {
                    let name = $(item).attr("data-name"); //item.innerText;
            ...
            let enemiesInHospRows = $("#enemiesList > div.enemies-rows > div.row > div:nth-child(2)").filter(function(idx) {
                return $(this).text().indexOf('ospital') > -1;
            }).closest(".row");
            let enemiesInHospStatus = $("#enemiesList > div.enemies-rows > div.row > div:nth-child(2)").filter(function(idx) {
                return $(this).text().indexOf('ospital') > -1;
            });

            let enemiesNotInHospRows = $("#enemiesList > div.enemies-rows > div.row > div:nth-child(2)").filter(function(idx) {
                return $(this).text().indexOf('ospital') == -1;
            }).closest(".row");
            let enemiesNotInHospStatus = $("#enemiesList > div.enemies-rows > div.row > div:nth-child(2)").filter(function(idx) {
                return $(this).text().indexOf('ospital') == -1;
            });
    ...
    */
    ...
    function filterSellItemList(retries=0) {
        ...
        for (let idx=0; idx<filtKeys.length; idx++) {
            ...
            let node = `<label>${key}<input id="${key}-filt" class="ifilt" type="checkbox" name="${entry.name}" data-type="${dataType}"></label>`;

            //$(node).find("input").prop('checked', entry.isChecked);
            $("#sell-filter-items").append($(node));
            $(`"#${key}-filt"`).prop('checked', entry.isChecked);
        }
