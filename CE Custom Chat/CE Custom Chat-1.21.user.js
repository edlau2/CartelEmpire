...
(function() {
    ...
    function initSettings(optObject) {
        debug("[initSettings] can init properties AFTER this call. Settings: ", settings);
        ...
        for (let idx=0; idx<keys.length; idx++) {
            ...
            if (key == 'chatbox-bg-opacity' && Object.keys(value) < 4) {
                GM_deleteValue(key);
            }
            // Save defaults if not set
            let tmp  = JSON.parse(GM_getValue(key, JSON.stringify(value)));
            debug("[initSettings] key: ", key, " tmp: ", tmp);
            debug("[initSettings] settings[key]: ", settings[key]);
            settings[key] = tmp; //.parse(GM_getValue(key, JSON.stringify(value)));
    ...
    function initDefProps() {
        debug("[initDefProps] optsInit: ", optsInit);
        if (optsInit == false) {
            console.error("CE Custom Chat [initDefProps] opts not initialized!!");
            return log("ERROR opts not initialized!!");
        }
...
/*                 if (options.chatActiveOpacity == true) {
    ...
    function installChatSettings(retries=0) {
        ...
        function getSettingsRow(key) {
            ...
            let numInput = `<td><span class="settingsVal">
                               <input class="settings-input text-center text-xl-start" name="${key}" type="number" value="${entry.value}">
                            </span></td>`;
            let textInput = `<td><span class="settingsVal">
                               <input class="settings-input text-center text-xl-start" name="${key}" type="text" value="${entry.value}">
                            </span></td>`;

            let addlClass = ``;
            if (entry.value) addlClass = "flex-r";

             let newRow =
                  `<tr id="${key}" class="align-middle settings-row" style="border-bottom-width: 1px;">
                      <td class="${addlClass}"><span>${entry.desc}</span></td>
                  </tr>`;
            ...
            switch (style) {
                case "color": {
                    newRow = `<tr class="align-middle settings-row" style="border-bottom-width: 1px;">
                                  <td class="flex-r"><span>${entry.desc}</span><span class="settingsVal">
                                       <input class="chat-w-over settings-input text-center text-xl-start" name="${key}" type="text" value="${entry.value}">
                                   </span></td>
                ...
                case "number": {
                    newRow = `<tr class="align-middle settings-row" style="border-bottom-width: 1px;">
                                  <td class="flex-r"><span>${entry.desc}</span><span class="settingsVal">
                                       <input class="chat-w-over settings-input text-center text-xl-start" name="${key}" type="number" value="${entry.value} min="${entry.min}" max="${entry.max}">
                                   </span></td>
                                   <td><div class="form-check form-switch">
                                        <input class="form-check-input" name="${key}" type="checkbox" id="${key}">
                                   </div></td>
