...
(function() {
    ...
    function initSettings(optObject) {
        $.each(optObject, function(key, value) {
            settings[key] = JSON.parse(GM_getValue(key, JSON.stringify(value)));
    ...
    const settings = {};
    const defSettings = {
        "backgroundOpacity": { "on": true, "desc": "Opacity of the chat box background", "visible": true, "grp": "app" },
        "myTextColor":  { "on": true, "desc": "Color of my message text (visible only to me)", "visible": true, "grp": "app" },
        "applyBorders":  { "on": true, "desc": "Place border boxes around my messages (visible only to me)", "visible": true, "grp": "app" },
        "myBorderColor":  { "on": true, "desc": "Color of my message borders (visible only to me)", "visible": true, "grp": "app" },
    }
